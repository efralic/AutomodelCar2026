...
int DBSCAN::expandCluster(PointC point, int clusterID){
    ...
    for(size_t i = 0, n = seeds.size(); i < n; ++i){
        std::vector<int> neighbors = calculateCluster(m_points.at(seeds[i]));
...
void DBSCAN::getCluster(std::map<int, std::vector<cv::Point>> &clusters_points){
    for(auto p : m_points)
        clusters_points[p.clusterID].push_back(cv::Point((int)p.x,(int)p.y));
}
...
public:
    ObjectDetection(): nh(){
        ...
        ROS_INFO("ObjectDetection ready. Listening to /scan (RPLIDAR C1)");
    }
    ...
    void laser_msg_Callback(const sensor_msgs::LaserScan::ConstPtr& scan){
        ...
        // ── FIX: usar índice entero, no float ────────────────────────────
        // El RPLIDAR C1 publica ~360 puntos por vuelta (ángulo_min=-π,
        // incremento≈1°). Calculamos el índice correcto para cada ángulo
        // deseado en lugar de usar el ángulo como índice directamente.
        ...
        // Convertir límites de ROI de grados a radianes
        float roi_start_rad = INITIAL_RANGE_ANGLE * M_PI / 180.0f;
        float roi_end_rad   = END_RANGE_ANGLE      * M_PI / 180.0f;
        ...
        if(points_centroids.size() > 0){
            ...
            for(auto point : points_centroids){
                ...
                ROS_INFO("Obstacle: dist=%.2f cm, angle=%.1f deg",
    ...
    void get_object_points(const vector_point& scan_points){
        ...
        for(auto point : scan_points){
            r = point.y * 100.0f;   // metros → cm
            // point.x = ángulo en grados
            x = r * cos((180.0f - point.x) * M_PI / 180.0f);
            y = r * sin((180.0f - point.x) * M_PI / 180.0f);
            x = 400.0f + x;
            x = (x < 800.0f) ? x : 800.0f;
            x = (x >= 0.0f)  ? x : 0.0f;
            y = 400.0f + y;
    ...
    void transform_point(cv::Point &point, geometry_msgs::Point &point_msg){
        float adjacent_leg = point.y - 400;
        float opposite_leg = point.x - 400;
        adjacent_leg = (adjacent_leg == 0.0f) ? 0.000001f : adjacent_leg;
        if(point.x >= 0 && point.x <= 400 && point.y >= 0 && point.y <= 400)
            point_msg.y = atan(opposite_leg/adjacent_leg)*57.295779f;
        else if(point.x >= 0 && point.x <= 400 && point.y >= 400 && point.y <= 800)
            point_msg.y = 180.0f + atan(opposite_leg/adjacent_leg)*57.295779f;
        else if(point.x >= 400 && point.x <= 800 && point.y >= 400 && point.y <= 800)
            point_msg.y = 180.0f + atan(opposite_leg/adjacent_leg)*57.295779f;
        else if(point.x >= 400 && point.x <= 800 && point.y >= 0 && point.y <= 400)
            point_msg.y = 360.0f + atan(opposite_leg/adjacent_leg)*57.295779f;
    ...
    void get_centroids_objects(const std::map<int,std::vector<cv::Point>> &cp){
        ...
        for(auto cluster : cp){
            if(cluster.first == NOISE) continue;   // ignorar ruido
...
int main(int argc, char **argv){
    ...
    ROS_INFO("Object detection node running (RPLIDAR C1)...");
